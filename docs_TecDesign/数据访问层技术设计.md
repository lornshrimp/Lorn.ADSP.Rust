...
本设计将原.NET方案全面迁移到 Rust 技术栈，围绕“透明化数据访问 + 缓存优先 + 运行时可切换数据源”的目标，基于项目现有统一配置与依赖注入基础设施，实现高性能、低延迟、标准化、可演进的数据访问层。

适用技术栈：Rust 1.75+ / Tokio / SeaORM / Redis / rdkafka or lapin（可选）/ tracing / OpenTelemetry。

与本项目对齐：参考 README 与 系统整体技术架构；遵循 docs 规范：技术设计文档仅包含架构与接口定义，不包含具体实现代码。

---

## 1. 范围与目标

- 对上层完全透明：屏蔽缓存与数据库细节，提供统一 trait 抽象。
- 缓存优先：默认 Cache→DB→回写（可策略化）。
- 运行时可选：通过配置与DI在运行时选择 MySQL/PostgreSQL、Redis 等实现。
- 无环依赖：严格按工作空间分层依赖向内汇聚。
- 标准化：仓储/规约/工作单元可选，首选 SeaORM 原生事务与查询能力。
...
- crates/04-core/
    - domain：领域模型与仓储接口（DDD核心，定义业务仓储trait）。
    - shared：跨层共享类型（错误、结果、时间等）。

- crates/05-infrastructure/
    - common（已存在）：通用工具与组件。
    - config-abstractions、config-impl（已存在）：统一配置抽象与实现。
    - di-abstractions、di-impl（已存在）：统一依赖注入抽象与实现。
    - composition（已存在）：基础设施组合与组件发现（examples 已验证）。
    - data-abstractions（待建）：数据访问抽象层（与DB/缓存无关的统一trait与模型）。
    - data-postgresql（待建）：PostgreSQL实现（SeaORM）。
    - data-mysql（待建，可后续）：MySQL实现（SeaORM）。
    - caching-abstractions（待建）：缓存抽象（键控缓存、对象缓存、批量、失效、命名空间等）。
    - caching-redis（待建）：Redis实现。

依赖方向：presentation/services → 04-core → 05-infrastructure（向内依赖）；抽象高于实现；组合在 composition 完成，应用仅引入 composition。
...
C4Context
title 数据访问 - C4 容器视图
Person(User, 业务服务/策略引擎)
Container(App,"Web/API/服务进程","Axum/Tokio","处理业务流与策略")
Container_Boundary(Infra,"05-infrastructure"){
    Container(DAAbs,"data-abstractions","crate","数据访问抽象trait/模型")
    Container(DApg,"data-postgresql","crate","SeaORM + Postgres 实现")
    Container(DAmysql,"data-mysql","crate","SeaORM + MySQL 实现")
    Container(CacheAbs,"caching-abstractions","crate","缓存抽象")
    Container(CacheRedis,"caching-redis","crate","Redis实现")
    Container(Config,"config-impl","crate","统一配置")
    Container(DI,"di-impl","crate","依赖注入/组件发现")
    Container(Compose,"composition","crate","组合与装配入口")
}
Rel(User,App,"请求数据")
Rel(App,Compose,"装配","启动时")
Rel(App,DAAbs,"通过trait访问")
Rel(DAAbs,CacheAbs,"缓存优先")
Rel(CacheAbs,CacheRedis,"运行时选择实现")
Rel(DAAbs,DApg,"回退到DB")
Rel(DAAbs,DAmysql,"可选实现")
Rel(DAAbs,Config,"读取策略/连接")
Rel(DAAbs,DI,"解析实现")
```

---

## 4. 核心抽象与契约（Rust trait）

说明：以下为接口与数据结构设计，用于指导实现；不包含具体代码实现。

### 4.1 访问上下文与键

用途：统一封装访问键、命名空间、租户、幂等与追踪信息。

```rust
pub struct DataKey {
    pub ns: String,          // 命名空间（业务域）
    pub key: String,         // 业务键，如 "ad:123"
}

pub struct DataAccessCtx {
    pub tenant_id: Option<String>,
    pub trace_id: Option<String>,
    pub deadline_ms: Option<u64>,   // 超时预算
}
```
...
pub trait CacheProvider {
    async fn get<T: serde::de::DeserializeOwned + Send>(&self, key: &DataKey) -> Result<Option<T>, CacheError>;
    async fn set<T: serde::Serialize + Send>(&self, key: &DataKey, value: &T, ttl_sec: Option<u64>) -> Result<(), CacheError>;
    async fn exists(&self, key: &DataKey) -> Result<bool, CacheError>;
    async fn remove(&self, key: &DataKey) -> Result<(), CacheError>;
}
```
...
pub trait DbSession: Send + Sync {
    fn backend(&self) -> DbBackendKind; // Postgres / MySql
}

#[async_trait::async_trait]
pub trait TransactionManager {
    async fn with_txn<F, Fut, T>(&self, f: F) -> Result<T, DbError>
    where F: FnOnce() -> Fut + Send,
                Fut: std::future::Future<Output = Result<T, DbError>> + Send,
                T: Send;
}
```

说明：具体查询通过各仓储在 SeaORM Entity/ActiveModel 上实现；此处抽象事务与会话边界，避免上层感知数据库方言。

### 4.4 数据访问路由与提供者链

```rust
pub enum RoutingStrategy { CacheFirstThenDb, DbFirstThenCache, CacheOnly, DbOnly }

pub struct ProviderChainNode { pub role: ProviderRole, pub priority: u8 }

#[async_trait::async_trait]
pub trait DataAccessRouter {
    async fn build_chain(&self, key: &DataKey, ctx: &DataAccessCtx) -> Vec<ProviderChainNode>;
    fn strategy_for(&self, ns: &str) -> RoutingStrategy;
}

#[async_trait::async_trait]
pub trait ProviderChainExecutor {
    async fn get<T: serde::de::DeserializeOwned + Send + 'static>(&self, key: &DataKey, ctx: &DataAccessCtx) -> Result<Option<T>, DataError>;
    async fn set<T: serde::Serialize + Send + 'static>(&self, key: &DataKey, ctx: &DataAccessCtx, value: &T, ttl_sec: Option<u64>) -> Result<(), DataError>;
}
```

### 4.5 仓储/规约/工作单元（可选）

建议：领域仓储 trait 放在 crates/04-core/domain；基础设施实现落在 data-*/caching-*。
...
pub trait Specification<T> { fn matches(&self, item: &T) -> bool; }
```
...
sequenceDiagram
    ...
    participant X as ProviderChainExecutor
    participant C as CacheProvider
    participant D as DB(SeaORM)
    ...
    alt 命中
        ...
        X->>D: 查询
        D-->>X: Some(value)
        X->>C: set(key,value,ttl)
        X-->>R: Some(value)
        R-->>S: 返回
    end
```

回写策略：默认同步回写；高压场景可采用异步回写（配置化）。

---

## 6. 配置与依赖注入

使用 crates/05-infrastructure/config-impl 与 di-impl，组合于 composition：

- 配置（示例，toml/json 等均可）：

```toml
[data_access]
default_strategy = "CacheFirstThenDb"
...
url = "postgres://user:pass@localhost:5432/adsp"
pool_max = 50
command_timeout_ms = 30000

[routing]
Advertisement = "CacheFirstThenDb"
UserProfile = "CacheOnly"
Targeting = "DbFirstThenCache"
Delivery = "DbOnly"
```

- 组件发现与装配：
    - caching-redis 实现 CacheProvider，并以组件标签导出。
    - data-postgresql 实现 DbSession/TransactionManager 与各仓储实现。
    - composition 在启动时按命名/标签注册进 DI；runtime 依据配置选择实现。

---

## 7. SeaORM 建模与迁移

- 实体与迁移：为每个数据库实现 crate 提供 sea-orm-entity 与 sea-orm-migration 子模块或依赖。
- 迁移策略：
    - 本地开发：sea-orm-cli/migration 运行；
    - CI：执行 dry-run + 集成测试；
    - 生产：受控窗口/灰度，严格备份与回滚脚本。
- 版本管理：语义化命名，按服务/领域拆分。
...
- 事务边界：以请求/聚合根操作为单位，TransactionManager.with_txn 包裹。
- 隔离级别：默认数据库级（READ COMMITTED/REPEATABLE READ）按后端配置；避免长事务。
- 缓存一致性：
    - 写入后：先DB成功→再缓存更新（写后写）；
    - 失效策略：TTL + 精确失效（命名空间key模式），必要时订阅失效广播；
    - 跨表更新：推荐 outbox pattern（事件写入DB后异步消费做扩散/失效）。

---

## 9. 错误处理与可观测性

- 错误分类：CacheError/DbError/RepoError/DataError；使用 thiserror 统一展示，anyhow 用于边界聚合。
- 追踪：tracing + OpenTelemetry，关键路径埋点（命中/未命中、延迟、失败类型）。
- 指标：
    - QPS、p50/p95、命中率、错误率；
    - 连接池占用、超时/重试次数；
    - 事务冲突与回滚计数。
...
```mermaid
classDiagram
    class DataAccessConfig { default_strategy: RoutingStrategy }
    class ProviderRegistry { resolve(redis), resolve(pg) }
    class CacheProvider <|.. RedisCacheProvider
    class DbSession <|.. PgSession
    class TransactionManager <|.. PgTxManager

    ProviderRegistry --> RedisCacheProvider
    ProviderRegistry --> PgSession
    ProviderRegistry --> PgTxManager
```

原则：
- 默认提供 PostgreSQL；MySQL 作为后续选项并行实现，均通过相同抽象暴露。
- 应用仅依赖抽象与 composition；不直接依赖具体 data-* 或 caching-*。

---

## 11. 安全与合规

- 机密管理：连接串与密码经统一配置系统注入，优先环境变量/密管（后续支持多云KV）。
- 最小权限：DB 账户按库/表/操作最小授权；Redis 使用前缀隔离与 ACL。
- 审计：关键数据读写、DDL 变更、失败事件审计日志。

---

## 12. 测试策略

- 单元测试：trait mock + 纯内存实现（如内存缓存）覆盖核心分支。
- 集成测试：testcontainers 启动 Postgres/Redis，覆盖事务、并发、失效链路。
- 性能基线：Criterion 对仓储关键查询与缓存链路建立基准，门槛回归守护。
...
graph TB
    subgraph core
        domain[04-core/domain]
        shared[04-core/shared]
    end
    subgraph infra
        common[05-infra/common]
        cfg[05-infra/config-*]
        di[05-infra/di-*]
        da_abs[05-infra/data-abstractions]
        da_pg[05-infra/data-postgresql]
        cache_abs[05-infra/caching-abstractions]
        cache_redis[05-infra/caching-redis]
        compose[05-infra/composition]
    end
    domain --> da_abs
    da_pg --> da_abs
    cache_redis --> cache_abs
    da_abs --> common
    cache_abs --> common
    compose --> cfg
    compose --> di
...
规则：
- 外层依赖内层，抽象不依赖实现；避免任何反向引用。

---

## 14. 开发实施指引

- 第1步：落地 data-abstractions（trait、模型、错误定义）。
- 第2步：实现 caching-redis（基于 redis 或 deadpool-redis），注册为 CacheProvider。
- 第3步：实现 data-postgresql（SeaORM连接、事务、仓储样例）。
- 第4步：在 composition 中装配默认链：CacheFirstThenDb；添加配置解析。
- 第5步：以“广告（Advertisement）”为样例仓储，建立端到端集成测试（含Redis、Postgres）。
...
sequenceDiagram
    ...
    S->>R: save(entity)
    R->>Tx: with_txn{ insert/update }
    Tx->>D: exec
    D-->>Tx: ok
    Tx-->>R: ok
    R->>C: remove/refresh related keys
    R-->>S: ok
```
...
- 从.NET/EF迁移至 Rust/SeaORM；
- 以trait抽象替代接口/类层次；
- 默认 Postgres 优先，MySQL 并行规划；
- 缓存优先策略与回写/失效配置化；
- 组合注册通过 composition 统一装配，应用仅依赖 composition。

---

本设计文档专注技术设计与实现指导，不包含具体实现代码；实现需严格遵循crate依赖与命名规则，避免循环依赖，并保持与 docs_TecDesign 其他文档一致。
...
classDiagram
    ...
    class IDataAccessProviderChain {
        <<interface>>
        +Task~DataExecutionResult~T~~ ExecuteGetAsync~T~(context, providerChain)
        +Task~DataExecutionResult~bool~~ ExecuteSetAsync~T~(context, data, providerChain)
        +CacheWriteBackStrategy WriteBackStrategy
    }
    
    class BusinessDataProviderBase {
        <<abstract>>
        #Task~DataProviderResult~T~~ GetAsync~T~(context)
        #Task~DataProviderResult~bool~~ SetAsync~T~(context, data)
        -IDataAccessRouter _router
        -ILogger _logger
    }
    
    class IDataAccessRouter {
        <<interface>>
        +Task~IDataAccessProvider~ RouteAsync(context)
        +Task~IEnumerable~IDataAccessProvider~~ GetCandidateProvidersAsync(context)
        +Task~IEnumerable~DataProviderChainNode~~ BuildProviderChainAsync(context)
    }
    
    class DataProviderChainNode {
        +IDataAccessProvider Provider
        +ProviderNodeRole Role
        +int Priority
    ...
    BusinessDataProviderBase --> IDataAccessRouter : uses
    IDataAccessRouter --> DataProviderChainNode : builds
    DataProviderChainNode --> IDataAccessProvider : contains
    IDataAccessProviderChain --> DataProviderChainNode : executes
    IDataAccessProvider --> DataProviderMetadata : provides
```

#### 提供者链架构核心组件

**IDataAccessProviderChain - 提供者链执行器**

提供者链执行器是数据访问层的核心执行引擎，实现了标准的**缓存优先→数据库回退→缓存回写**访问模式：

```csharp
/// <summary>
/// 数据访问提供者链接口
/// 实现缓存优先的分层数据访问模式，支持透明的缓存管理和数据一致性保障
/// </summary>
public interface IDataAccessProviderChain : IHealthCheck, IDataProviderComponent
{
    /// <summary>
    /// 执行获取数据操作 - 实现缓存优先→数据库回退→缓存回写的执行链
    /// </summary>
    Task<DataExecutionResult<T>> ExecuteGetAsync<T>(
        DataAccessContext context,
        IEnumerable<DataProviderChainNode> providerChain,
        CancellationToken cancellationToken = default) where T : class;

    /// <summary>
    /// 执行设置数据操作 - 实现数据库写入→缓存同步更新的执行链
    /// </summary>
    Task<DataExecutionResult<bool>> ExecuteSetAsync<T>(
        DataAccessContext context,
        T data,
        IEnumerable<DataProviderChainNode> providerChain,
        CancellationToken cancellationToken = default) where T : class;
...
**BusinessDataProviderBase - 业务数据提供者抽象基类**

专门为业务数据提供者设计的抽象基类，**不适用于技术提供者**（如缓存、数据库提供者）。自动集成提供者链执行逻辑：

```csharp
/// <summary>
/// 业务数据提供者抽象基类
/// 注意：此基类专门为业务数据提供者设计，不适用于技术提供者
/// </summary>
public abstract class BusinessDataProviderBase : IDataAccessProvider, IHealthCheck, IDataProviderComponent
{
    /// <summary>
    /// 基于提供者链的数据获取 - 业务提供者调用此方法即可获得完整的缓存+数据库访问能力
    /// </summary>
    protected async Task<DataProviderResult<T>> GetAsync<T>(
        DataAccessContext context,
        CancellationToken cancellationToken = default) where T : class
    {
        // 1. 自动构建最优提供者链
        var providerChain = await _router.BuildProviderChainAsync(context, cancellationToken);
        
        // 2. 执行链式访问
        var chainExecutor = ServiceLocator.GetService<IDataAccessProviderChain>();
        var result = await chainExecutor.ExecuteGetAsync<T>(context, providerChain, cancellationToken);
...
**增强的IDataAccessRouter接口**

路由接口增加了提供者链构建能力，同时保留原有的单点路由功能：

```csharp
public interface IDataAccessRouter : IHealthCheck, IDataProviderComponent
{
    // 保留原有功能 - 向后兼容
    ...
    // 新增功能 - 提供者链构建
    Task<IEnumerable<DataProviderChainNode>> BuildProviderChainAsync(DataAccessContext context, CancellationToken cancellationToken = default);
}
```
...
sequenceDiagram
    ...
    participant Provider as 业务数据提供者
    participant Router as 路由决策引擎
    participant Chain as 提供者执行链
    participant Cache as 缓存提供者
    participant Database as 数据库提供者
    
    Client->>Provider: 请求广告数据
    Provider->>Router: 构建提供者链
    Router->>Router: 应用路由策略(CacheFirst)
    Router->>Chain: 创建执行链[Cache→Database]
    Router-->>Provider: 返回执行链
    
    Provider->>Chain: 执行数据获取
    Chain->>Cache: 尝试缓存访问
    alt 缓存命中
        Cache-->>Chain: 返回缓存数据
        Chain-->>Provider: 返回执行结果
        Provider-->>Client: 返回数据
    else 缓存未命中
        Chain->>Database: 访问数据库
        Database-->>Chain: 返回数据库结果
        Chain->>Cache: 异步回写缓存
        Chain-->>Provider: 返回执行结果
        Provider-->>Client: 返回数据
    end
